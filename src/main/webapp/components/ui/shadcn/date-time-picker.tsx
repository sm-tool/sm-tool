...
import { CalendarProps } from '@/components/ui/shadcn/calendar.tsx';
...
function genMonths(
  ...
  return Array.from({ length: 12 }, (_, index) => ({
    value: index,
    label: format(new Date(2021, index), 'MMMM', { locale }),
  }));
}

function genYears(yearRange = 50) {
  const today = new Date();
  return Array.from({ length: yearRange * 2 + 1 }, (_, index) => ({
    value: today.getFullYear() - yearRange + index,
    label: (today.getFullYear() - yearRange + index).toString(),
  }));
}
...
function Calendar({
  ...
  const MONTHS = React.useMemo(() => {
    ...
    const { options, localize, formatLong } = properties.locale || {};
  ...
  const disableLeftNavigation = () => {
    ...
    if (properties.month) {
      return (
        properties.month.getMonth() === startDate.getMonth() &&
        properties.month.getFullYear() === startDate.getFullYear()
      );
    }
    return false;
  };
  const disableRightNavigation = () => {
    ...
    if (properties.month) {
      return (
        properties.month.getMonth() === endDate.getMonth() &&
        properties.month.getFullYear() === endDate.getFullYear()
      );
    }
    return false;
  };
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        weekdays: cn('flex', properties.showWeekNumber && 'justify-end'),
        weekday:
            ...
            <div className='inline-flex gap-2'>
              <Select
                ...
                onValueChange={value => {
                  ...
                  properties.onMonthChange?.(newDate);
                }}
              >
...
interface PeriodSelectorProperties {
  period: Period;
...
interface TimePickerInputProperties
  ...
  (
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (['ArrowUp', 'ArrowDown'].includes(e.key)) {
        ...
        const temporaryDate = date ? new Date(date) : new Date();
        onDateChange?.(setDateByType(temporaryDate, newValue, picker, period));
      }
      if (e.key >= '0' && e.key <= '9') {
        if (picker === '12hours') setPreviousIntKey(e.key);
...
interface TimePickerReference {
  minuteRef: HTMLInputElement | null;
  hourRef: HTMLInputElement | null;
  secondRef: HTMLInputElement | null;
}

const TimePicker = React.forwardRef<TimePickerReference, TimePickerProperties>(
  ({ date, onChange, hourCycle = 24, granularity = 'second' }, reference) => {
    const minuteReference = React.useRef<HTMLInputElement>(null);
    const hourReference = React.useRef<HTMLInputElement>(null);
    const secondReference = React.useRef<HTMLInputElement>(null);
    const periodReference = React.useRef<HTMLButtonElement>(null);
    ...
    useImperativeHandle(
      reference,
      () => ({
        minuteRef: minuteReference.current,
        hourRef: hourReference.current,
        secondRef: secondReference.current,
        periodRef: periodReference.current,
      }),
      [minuteReference, hourReference, secondReference],
    );
    return (
        ...
        {(granularity === 'minute' || granularity === 'second') && (
          <>
            ...
            <TimePickerInput
              ...
              onRightFocus={() => secondReference?.current?.focus()}
            />
          </>
        )}
        {granularity === 'second' && (
          <>
            ...
            <TimePickerInput
              ...
              onDateChange={onChange}
              ref={secondReference}
              onLeftFocus={() => minuteReference?.current?.focus()}
              onRightFocus={() => periodReference?.current?.focus()}
            />
          </>
        )}
        {hourCycle === 12 && (
          <div className='grid gap-1 text-center'>
            <TimePeriodSelect
              ...
              ref={periodReference}
              onLeftFocus={() => secondReference?.current?.focus()}
            />
          </div>
        )}
      </div>
...
type DateTimePickerReference = {
  value?: Date;
} & Omit<HTMLButtonElement, 'value'>;

const DateTimePicker = React.forwardRef<
  Partial<DateTimePickerReference>,
...
export type {
  TimePickerType,
  DateTimePickerProperties as DateTimePickerProps,
  DateTimePickerReference as DateTimePickerRef,
};
